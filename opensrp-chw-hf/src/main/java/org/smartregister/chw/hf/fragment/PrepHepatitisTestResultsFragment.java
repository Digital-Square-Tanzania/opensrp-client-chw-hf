...
public class PrepHepatitisTestResultsFragment extends BaseTestResultsFragment {

    public static final String BASE_ENTITY_ID = "BASE_ENTITY_ID";
    private long mLastClickTime = 0;
    private String baseEntityId;

    public static PrepHepatitisTestResultsFragment newInstance(String baseEntityId) {
        PrepHepatitisTestResultsFragment hvlResultsFragment = new PrepHepatitisTestResultsFragment();
        Bundle b = new Bundle();
    ...
    protected void initializePresenter() {
        ...
        presenter = new PrepHepatitisAndCrclTestResultsFragmentPresenter(baseEntityId, this, new KvpHepatitisTestResultsFragmentModel(), null);
    }
    ...
    public void openResultsForm(CommonPersonObjectClient client) {
        ...
        if (testType.contains("hep_b")) {
            ...
            try {
                JSONObject jsonObject = (new FormUtils()).getFormJsonFromRepositoryOrAssets(requireContext(), Constants.FORMS.PREP_HBV_RESULTS);
            ...
            try {
                JSONObject jsonObject = (new FormUtils()).getFormJsonFromRepositoryOrAssets(requireContext(), Constants.FORMS.PREP_HCV_RESULTS);
                assert jsonObject != null;
                KvpTestResultsViewActivity.startResultsForm(getContext(), jsonObject.toString(), baseEntityId, entityId);
            } catch (JSONException e) {
                Timber.e(e);
            }
        } else if (testType.contains("crcl")) {
            String baseEntityId = Utils.getValue(client.getColumnmaps(), DBConstants.KEY.BASE_ENTITY_ID, false);
            String entityId = Utils.getValue(client.getColumnmaps(), DBConstants.KEY.ENTITY_ID, false);

            try {
                JSONObject jsonObject = (new FormUtils()).getFormJsonFromRepositoryOrAssets(requireContext(), Constants.FORMS.PREP_CRCL_RESULTS);
                assert jsonObject != null;
                KvpTestResultsViewActivity.startResultsForm(getContext(), jsonObject.toString(), baseEntityId, entityId);
            } catch (JSONException e) {
                Timber.e(e);
    ...
    private void openEditResultsForm(CommonPersonObjectClient client) {
        ...
        try {
            ...
            String testType = Utils.getValue(client.getColumnmaps(), DBConstants.KEY.HEPATITIS_TEST_TYPE, false);
            if (testType.contains("hep_b")) {
                jsonObject = (new FormUtils()).getFormJsonFromRepositoryOrAssets(requireContext(), Constants.FORMS.PREP_HBV_RESULTS);
            } else if (testType.contains("hep_c")) {
                jsonObject = (new FormUtils()).getFormJsonFromRepositoryOrAssets(requireContext(), Constants.FORMS.PREP_HCV_RESULTS);
            } else if (testType.contains("crcl")) {
                jsonObject = (new FormUtils()).getFormJsonFromRepositoryOrAssets(requireContext(), Constants.FORMS.PREP_CRCL_RESULTS);
            }
